            ...
            parallel {
                stage('Build docker') {
                    ...
                    stages {
                        ...
                        stage('build') {
                            steps {
                                sh """
                                    ...
                                    echo "${ARTI_URL_WITH_TOKEN}/api/conda/conda-forge-remote" > ./arti_conda_repo.txt
                                ...
                                script {
                                    docker.withRegistry(ARTI_URL, 'OBS2CO_ARTIFACTORY_TOKEN') {
                                        ...
                                        #transmission des credentials proxy à l'image en passant par le système de secrets
                                            DOCKER_BUILDKIT=1 docker build -t artifactory.cnes.fr/obs2co-docker/sisppeo:latest --no-cache \
                            ...
                            steps {
                                script {
                                    docker.withRegistry(ARTI_URL, 'OBS2CO_ARTIFACTORY_TOKEN') {
                                        ...
                                        docker tag artifactory.cnes.fr/obs2co-docker/sisppeo:latest artifactory.cnes.fr/obs2co-docker/sisppeo:\$VERSION

                                        jfrog rt docker-push --skip-login --server-id ${SERVERID} artifactory.cnes.fr/obs2co-docker/sisppeo:\$VERSION obs2co-docker
                                        jfrog rt docker-push --skip-login --server-id ${SERVERID} artifactory.cnes.fr/obs2co-docker/sisppeo:latest obs2co-docker
			            ...
			            //Erreur lors de l'import de sisppeo 
                        stage('test') {
                            steps {
                                script {
                                    docker.withRegistry(ARTI_URL, 'OBS2CO_ARTIFACTORY_TOKEN') {
                                        sh  """
                                        # Publie sur Artifactory
                                        #docker pull artifactory.cnes.fr/obs2co-docker/sisppeo:latest
            
                                        # Test avec une image du datalake
                                        #docker run --name sisppeo -v /datalake:/datalake artifactory.cnes.fr/obs2co-docker/sisppeo:${env.BUILD_NUMBER} python /data/launcher.py /data/config.yml
                                        #sleep 5
                                        #docker logs sisppeo
                                        """
                                    }
