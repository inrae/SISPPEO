...
# TODO EFR
# TODO WFR
# TODO voir si c'est pas le même fonctionnement que MODIS cad que la grille d'image soit du 1°x1°
import warnings
...
from sisppeo.utils.main import format_zippath
...
class S3ESAReader(Reader):
    ...
    def extract_bands(self) -> None:
        ...
        if self._inputs.input_product.suffix == '.zip':
            ...
            dataset = rasterio.open(format_zippath(self._inputs.input_product) + xml_path)
        else:
            dataset = rasterio.open(list(self._inputs.input_product.glob('MTD_MSI*.xml'))[0])
            ...
            with rasterio.open(path) as subdataset:
                for i, band in tqdm(bands, unit='bands'):
                    ...
                    if self._inputs.out_resolution is None:
                        self._inputs = self._inputs._replace(out_resolution=subdataset.res[0])
                    if self._intermediate_data['x'] is None:
                        if ((out_res := self._inputs.out_resolution) > (in_res := subdataset.res[0])):
                        ...
                        band_array, xy_bbox = self._extract_first_band(subdataset, i)
                    else:
                        band_array = self._extract_nth_band(subdataset, i, xy_bbox)
        ...
        ds = xr.Dataset(
            ...
            coords={'x': ('x', self._intermediate_data['x']),
                    ...
                    'time': [datetime.fromisoformat(
                        self._intermediate_data['metadata']['tags']['PRODUCT_START_TIME'][:-1])]
                    }
        )
    ...
    def _extract_first_band(self, subdataset, i):
        if self._inputs.ROI is not None:
            ...
            arr = subdataset.read(i, window=Window.from_slices((row_start, row_stop + 1), (col_start, col_stop + 1)))
        ...
        if (out_res := self._inputs.out_resolution) != subdataset.res[0]:
            band_array = resample_band_array(band_array, subdataset.res[0], out_res)
    ...
    def _extract_nth_band(self, subdataset, i, xy_bbox):
        ...
        if (out_res := self._inputs.out_resolution) != subdataset.res[0]:
            band_array = resize_and_resample_band_array(band_array, ij_bbox, subdataset.res[0], out_res)
        return band_array
